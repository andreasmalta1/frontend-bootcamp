function clearAll() {
  containerEls = document.querySelectorAll('.container');
  containerEls.forEach(node => node.innerHTML = "");
}
...
function showJoke() {
  ...
  pEl.textContent = randomJokeText;

  jokeEl.appendChild(pEl);
}
...
function showRiddle() {
  ...
  questionEl.textContent = randomRiddle.question;
  questionEl.setAttribute("id", "question");
  
  riddleEl.appendChild(questionEl);
 }
...
function revealAnswers() {
  ...
  if (!questionEL){
    window.alert("Reveal a question first")
    return
  }
  ...
  for (let i = 0; i < riddles.length; i++) {
    console.log(riddles[i].question)
    if (riddles[i].question === question) {
      console.log('hello')
  ...
  answerEl.setAttribute("id", "answer");
  
  riddleEl.appendChild(answerEl);
}
...
const quotes = [
 ...
 const riddles = [
  ...
  {
    question: "You measure my life in hours and I serve you by expiring. I'm quick when I'm thin and slow when '’m fat. The wind is my enemy. ",
    answer: "A Candle",
  },
  {
